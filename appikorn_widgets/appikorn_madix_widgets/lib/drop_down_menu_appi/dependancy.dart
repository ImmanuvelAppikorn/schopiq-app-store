...
_SearchMatchFn _defaultSearchMatchFn = (item, searchValue) => item.value.toString().toLowerCase().contains(searchValue.toLowerCase());
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final double start = _clampDouble(0.5 + (widget.itemIndex + 1) * unit, 0.0, 1.0);
    ...
    Widget child = Container(
      ...
      height: widget.customItemsHeights == null ? widget.route.itemHeight : widget.customItemsHeights![widget.itemIndex],
      child: widget.route.items[widget.itemIndex],
    );
    // An [InkWell] is added to the item only if it is enabled
    // isNoSelectedItem to avoid first item highlight when no item selected
    if (dropdownMenuItem.enabled) {
      final isSelectedItem = !widget.route.isNoSelectedItem && widget.itemIndex == widget.route.selectedIndex;
      child = InkWell(
        ...
        child: Container(
          color: isSelectedItem ? widget.route.selectedItemHighlightColor : null,
          child: child,
        ),
      );
    }
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  List<Widget> _getSearchItems() {
    return <Widget>[
      for (int index = 0; index < widget.route.items.length; ++index)
        if (_searchMatchFn(widget.route.items[index].item!, widget.searchController!.text))
          _DropdownMenuItemButton<T>(
            route: widget.route,
            padding: widget.padding,
            buttonRect: widget.buttonRect,
      ...
      child: CustomPaint(
        ...
        child: Semantics(
          ...
          child: ClipRRect(
            ...
            clipBehavior: widget.dropdownDecoration?.borderRadius != null ? Clip.antiAlias : Clip.none,
            borderRadius: widget.dropdownDecoration?.borderRadius?.resolve(Directionality.of(context)) ?? BorderRadius.zero,
            child: Material(
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    final _MenuLimits menuLimits = route.getMenuLimits(buttonRect, availableHeight, route.selectedIndex);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final actualConstraints = constraints.copyWith(maxHeight: constraints.maxHeight - MediaQuery.of(context).viewInsets.bottom);
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    ...
    final double selectedItemOffset = getItemOffset(
      index,
      dropdownPadding != null ? dropdownPadding!.resolve(null).top : kMaterialListPadding.top,
    );
    ...
    double preferredMenuHeight = dropdownPadding?.vertical ?? kMaterialListPadding.vertical;
    if (items.isNotEmpty) {
...
class DropdownMenu<T> extends StatefulWidget {
  DropdownMenu({
    ...
    // DropdownButtonFormField.
        ...
        assert(
          customItemsHeights == null || items == null || items.isEmpty || customItemsHeights.length == items.length,
          "customItemsHeights list should have the same length of items list",
        ),
        formFieldCallBack = null;

  DropdownMenu._formField({
    super.key,
    required this.items,
    this.selectedItemBuilder,
    this.value,
...
class DropdownMenuState<T> extends State<DropdownMenu<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  void _updateSelectedIndex() {
    if (widget.items == null || widget.items!.isEmpty || (widget.value == null && widget.items!.where((DropdownMenuItem<T> item) => item.enabled && item.value == widget.value).isEmpty)) {
      _selectedIndex = null;
      return;
    }
  ...
  Rect _getRect() {
    ...
    final NavigatorState navigator = Navigator.of(context, rootNavigator: widget.dropdownFullScreen);
  ...
  double _getMenuHorizontalPadding() {
    final menuHorizontalPadding = (widget.itemPadding?.horizontal ?? _kMenuItemPadding.horizontal) + (widget.dropdownPadding?.horizontal ?? 0.0) + (widget.dropdownScrollPadding?.horizontal ?? 0.0);
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> items = widget.selectedItemBuilder == null ? (widget.items != null ? List<Widget>.of(widget.items!) : <Widget>[]) : List<Widget>.of(widget.selectedItemBuilder!(context));
    ...
    if (items.isEmpty) {
      ...
      innerItemsWidget = Padding(
        ...
        padding: EdgeInsets.symmetric(
          horizontal: widget.buttonWidth == null && widget.dropdownWidth == null ? _getMenuHorizontalPadding() : 0.0,
        ),
        child: IndexedStack(
      ...
      child: widget.customButton ??
          Container(
            decoration: widget.buttonDecoration?.copyWith(
              color: Colors.green,
              boxShadow: widget.buttonDecoration!.boxShadow ?? kElevationToShadow[widget.buttonElevation ?? 0],
            ),
        ...
        child: Theme(
          ...
          child: Material(
            ...
            child: InkWell(
              ...
              borderRadius: widget.buttonDecoration?.borderRadius?.resolve(Directionality.of(context)),
              splashColor: Colors.transparent,
              highlightColor: Colors.transparent,
              hoverColor: Colors.transparent,
              focusColor: Colors.transparent,
