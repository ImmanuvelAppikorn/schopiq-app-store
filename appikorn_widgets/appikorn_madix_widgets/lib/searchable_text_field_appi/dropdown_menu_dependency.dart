...
/// Defines a [DropdownMenuAppikorn] menu button that represents one item view in the menu.
///
/// See also:
///
/// * [DropdownMenuAppikorn]
class DropdownMenuEntryAppikorn<T> {
  /// Creates an entry that is used with [DropdownMenuAppikorn.dropdownMenuEntries].
  const DropdownMenuEntryAppikorn({
    required this.value,
    required this.label,
    this.labelWidget,
    this.leadingIcon,
  ...
  ///
  /// This value must be unique across all entries in a [DropdownMenuAppikorn].
  final T value;
  ...
  ///
  /// The default value is true. If true, the [DropdownMenuEntryAppikorn.label] will be filled
  /// out in the text field of the [DropdownMenuAppikorn] when this entry is clicked; otherwise,
...
///
/// The menu is composed of a list of [DropdownMenuEntryAppikorn]s. People can provide information,
...
///
/// {@tool dartpad}
/// This sample shows how to display outlined [DropdownMenuAppikorn] and filled [DropdownMenuAppikorn].
///
...
///   The [DropdownMenuAppikorn] uses a [TextField] as the "anchor".
/// * [TextField], which is a text input widget that uses an [InputDecoration].
/// * [DropdownMenuEntryAppikorn], which is used to build the [MenuItemButton] in the [DropdownMenuAppikorn] list.
class DropdownMenuAppikorn<T> extends StatefulWidget {
  /// Creates a const [DropdownMenuAppikorn].
  ///
  ...
  /// Determine if the [DropdownMenuAppikorn] is enabled.
  ///
  /// Defaults to true.
  final bool enabled;

  /// Determine the width of the [DropdownMenuAppikorn].
  ///
  /// If this is null, the width of the [DropdownMenuAppikorn] will be the same as the width of the widest
  ...
  /// Text that provides context about the [DropdownMenuAppikorn]'s value, such
  ...
  /// The text style for the [TextField] of the [DropdownMenuAppikorn];
  ///
  ...
  /// Descriptions of the menu items in the [DropdownMenuAppikorn].
  ///
  /// This is a required parameter. It is recommended that at least one [DropdownMenuEntryAppikorn]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<DropdownMenuEntryAppikorn<T>> dropdownMenuEntries;
  ...
  /// by the width of menu items or [DropdownMenuAppikorn.width]. If this property is not null,
  ...
  @override
  State<DropdownMenuAppikorn<T>> createState() => _DropdownMenuAppikornState<T>();
}

class _DropdownMenuAppikornState<T> extends State<DropdownMenuAppikorn<T>> {
  ...
  late List<DropdownMenuEntryAppikorn<T>> filteredEntries;
  List<Widget>? _initialMenu;
  int? currentHighlight;
  double? leadingPadding;
  bool _menuHasEnabledItem = false;
  final focus_scope = FocusNode();
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries.any((DropdownMenuEntryAppikorn<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere((DropdownMenuEntryAppikorn<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
      try {
    ...
    // Delay accessing the render object until after the first frame
  ...
  @override
  void didUpdateWidget(DropdownMenuAppikorn<T> oldWidget) {
  ...
  List<DropdownMenuEntryAppikorn<T>> filter(List<DropdownMenuEntryAppikorn<T>> entries, TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries.where((DropdownMenuEntryAppikorn<T> entry) => entry.label.toLowerCase().contains(filterText)).toList();
  }

  int? search(List<DropdownMenuEntryAppikorn<T>> entries, TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((DropdownMenuEntryAppikorn<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<DropdownMenuEntryAppikorn<T>> filteredEntries, TextEditingController textEditingController, TextDirection textDirection,
      {int? focusedIndex, bool enableScrollToHighlight = true}) {
    final List<Widget> result = <Widget>[];
    for (int i = 0; i < filteredEntries.length; i++) {
      final DropdownMenuEntryAppikorn<T> entry = filteredEntries[i];
      ...
      final Color focusedBackgroundColor = effectiveStyle.foregroundColor?.resolve(<WidgetState>{WidgetState.focused}) ?? Theme.of(context).colorScheme.onSurface;
      ...
      effectiveStyle = entry.enabled && i == focusedIndex ? effectiveStyle.copyWith(backgroundColor: WidgetStatePropertyAll<Color>(focusedBackgroundColor.withOpacity(0.12))) : effectiveStyle;
      ...
      final Widget menuItemButton = MenuItemButton(
        ...
        onPressed: entry.enabled
            ? () {
                print("this cicall edd ${entry.value}");
  ...
  void handleUpKeyInvoke(_) => setState(() {
        if (!_menuHasEnabledItem || !_isMenuOpen) {
          return;
        }
        _enableFilter = false;
        currentHighlight ??= 0;
  ...
  void _handleFocusChange() {
    if (!_menuFocusNode.hasFocus && _isMenuOpen) {
      Future.delayed(const Duration(milliseconds: 100), () {
        if (!mounted || _menuFocusNode.hasFocus) return;
        setState(() {
          _isMenuOpen = false;
  ...
  Widget build(BuildContext context) {
    ...
    final InputDecorationTheme effectiveInputDecorationTheme = widget.inputDecorationTheme ?? theme.inputDecorationTheme ?? defaults.inputDecorationTheme!;
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        ...
        final Widget textField = InkWell(
          ...
          onFocusChange: (S) {
            if (S == false) {
              ...
              focus_scope.requestFocus();
              _handleMenuOpen();
            }
          },
          child: TextField(
              key: _anchorKey,
              focusNode: focus_scope,
              ...
              onEditingComplete: () {
                if (currentHighlight != null) {
                  final DropdownMenuEntryAppikorn<T> entry = filteredEntries[currentHighlight!];
                  if (entry.enabled) {
